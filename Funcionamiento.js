...
let textarea = document.querySelector(".Presentacion__Contenido__entrada");
let mensaje = document.getElementById("outputText");
let placeholderImage = document.querySelector(".presentacion__imagen__desecriptado");
...
function encriptar() {
  ...
  // Rechazo de simbolos y letras mayusculas 
  if (/[^a-z\s?]/.test(texto)) {
...
function desencriptar() {
  let texto = textarea.value.trim();
    // Condicional de rechazo de simbolos y letras mayusculas 
  if (/[^a-z\s]/.test(texto)) {
    mensaje.textContent = 'El texto debe contener solo letras minúsculas y espacios.';
    placeholderImage.style.display = 'block';
    copyButton.style.display = 'none';
...
function copiar() {
  // aqui se obbtiene el texto que está en el mensaje (area de encriptado)
